 ...
 * and filter a collection with generics, however you can also use them without a UI, however some other
 * aspects needs to be discussed because they are strictly related to UI usage:
 * <p> Every filter has a name
 * <p> {@link BiPredicate}s are wrapped in a {@link BiPredicateBean}
 * <p> The BiPredicate to use is "selected" with an index property (ideal for combo boxes), see {@link #predicateFor(String)}.
 *
